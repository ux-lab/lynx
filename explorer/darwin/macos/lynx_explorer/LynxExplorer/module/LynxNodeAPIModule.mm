...
napi_value RequireNodeAddon(napi_env env, napi_callback_info info) {
  ...
  napi_get_value_string_utf8(env, argv[0], addon_name, sizeof(addon_name), nullptr);
  uint64_t token_id = reinterpret_cast<uint64_t>(data);
  lynx::explorer::LynxNodeAPI::GetInstance().RequireNodeAddonByToken(token_id,
...
napi_value LynxNodeAPIModuleCreator(napi_env env, napi_value exports, const char* module_name,
                                    void* opaque) {
  napi_value func;
  napi_create_function(env, "requireNodeAddon", NAPI_AUTO_LENGTH, &RequireNodeAddon, opaque, &func);
